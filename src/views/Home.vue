<template>
  ...
  <div class="outer" data-inset="1rem">
    ...
    >
      ...
      <div class="row vertical" :style="{'max-height':'400px', 'overflow-y':'auto'}">
...
export default {
  ...
  methods: {
    ...
    parseInfoContents() {
      ...
      Object.keys(this.dialogInfo).map(item => {
        if (this.dialogInfo[item] instanceof Array && !excArr.includes(item)) {
          this.dialogInfo[item] = this.dialogInfo[item].join(', ')
        }
      })
    },
